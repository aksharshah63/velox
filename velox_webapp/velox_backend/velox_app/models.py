...
from .morph import utils as morph_utils
from .common import score_tabular_prediction, score_tabular_classification_prediction
...
class GCSObject(models.Model):
    ...
    gcs_path = models.CharField('GCS Path', max_length=300, null=True, blank=True)
...
class Horse(CommonData):
    ...
    def update_status_new(self):
        ...
        if self.days_old > 2200:
            self.status = 'Retired'
        if self.start_days > 650:
            self.status = 'Retired'
        if self.starts >= 3 and self.avg_distance_raced:
...
class Measure(GCSObject, CommonData):
    ...
    dlc_video = GCSField('DLC Video', null=True, blank=True)
    dlc_h5_file = GCSField('DLC H5 File', null=True, blank=True)
    dlc_gaf_image = GCSField('DLC GAF Image', null=True, blank=True)
    dlc_jrg_image = GCSField('DLC JRG Image', null=True, blank=True)
    cardio_video = GCSField('Cardio Video', null=True, blank=True)
...
class ImageMeasurement(CommonData):
    ...
    def calculate_landmarks(self):
        ...
        if image:
            ...
            morp_res = morph_utils.prediction_from_bytes_to_dict(image_content)
...
class ScoreBins(CommonData):
    ...
    @classmethod
    def create_bins(cls, probability_field_name: str) -> ScoreBins:
        ...
        df_measuresx = df_measures[~df_measures[probability_field_name].isna()]
        df_measuresx = df_measuresx[probability_field_name].apply(lambda x: float(str(x)))
        res = pd.qcut(df_measuresx.unique(), q=6, retbins=True)
        bins_values = res[1]
        bins_data = {
            'D': [0.0, bins_values[1]],
