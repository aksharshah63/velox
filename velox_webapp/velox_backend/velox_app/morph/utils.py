    ...
    for i in range(0, len(images['im'])):
        ...
        if os.path.isfile(path) is True:
            ...
            if path in present_tags:
                ...
                images_e[pos].append(add_bbox_element(
                    images['coords'][i], sizes[name]))

            else:
                images_e.append(add_image_element(
                    name, images['coords'][i], sizes[name], path))

    et = ET.ElementTree(root)
    xmlstr = minidom.parseString(ET.tostring(
        et.getroot())).toprettyxml(indent="   ")
...
def split_train_test(input_dir):
    ...
    filenames = [os.path.join(input_dir, f)
    ...
    for split in ['train', 'test']:
        ...
        if not os.path.exists(split):
            ...
            print(
                "Warning: the folder {} already exists. It's being replaced".format(split))
            shutil.rmtree(split)
            os.mkdir(split)
        ...
        for filename in filenames[split]:
            ...
            res = image_prep(filename, name, split)
            if res is not None:
                sizes[split][name] = res
    return sizes
...
def predictions_to_xml(predictor_name: str, dir=str, ignore=None, out_file='output.xml'):
    ...
    Parameters:
        ...
        dir(str): name of the directory containing images to be predicted or a single image
    ...
    if os.path.isdir(dir):
        all_files = sorted(glob.glob(f'./{dir}/*'))
    else:
        all_files = [dir]
        # out_file = Path(dir).with_suffix('.xml')
        out_file = f'{Path(dir).stem}.xml'
    ...
    for f in all_files:
        ...
        if ext in extensions:
            ...
            e = (dlib.rectangle(left=1, top=1, right=w-1, bottom=h-1))
            ...
            for item, i in enumerate(sorted(part_length, key=str)):
                if ignore is not None:
                    if i not in ignore:
                        ...
                        part.set('y', str(h - int(shape.part(item).y)))
                        box.append(part)
                else:
                    part = ET.Element('part')
                    part.set('name', str(int(i)))
            ...
            box[:] = sorted(box, key=lambda child: (
                child.tag, float(child.get('name'))))
            image_e.append(box)
...
def prediction_to_dict(predictor_name: str, input_file_path=str, ignore=None):
    import cv2
    import dlib
    ...
    ext = ntpath.splitext(input_file_path)[1]
    if ext in extensions:
        ...
        out = dict()
        out['box'] = {}
        out['box']['top'] = str(int(1))
        out['box']['left'] = str(int(1))
        out['box']['width'] = str(int(img.shape[1] - 2))
        out['box']['height'] = str(int(img.shape[0] - 2))
        out['parts'] = {}
        part_length = range(0, shape.num_parts)
        for item, i in enumerate(sorted(part_length, key=str)):
            part = dict()
            if ignore is None or (ignore is not None and i not in ignore):
                part_name = int(i)
                part['x'] = int(shape.part(item).x)
                part['y'] = h - int(shape.part(item).y)
                out['parts'][part_name] = part

        return out


def prediction_from_bytes_to_dict(image_bytes: bytes, predictor_name: str = '', ignore=None) -> dict:
    import cv2
    import dlib

    if not predictor_name:
        folder = os.path.dirname(os.path.abspath(__file__))
        predictor_name = os.path.join(folder, 'predictor.dat')
        ...
        for images in root:
            for image in images:
                for boxes in image:
                    ...
                    for parts in boxes:
                        if parts.attrib['name'] is not None:
                            data = [float(parts.attrib['x']), float(
                                boxes.attrib['height']) + 2 - float(parts.attrib['y'])]
                        wr.writerows([data])
                    wr.writerows(
