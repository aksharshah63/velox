<template>
  <b-modal id="modalId" ref="modalRef" size="xl">
    ...
    <b-tabs content-class="mt-3">
      ...
      <b-tab title="Image" @click="tab_type('image')">
        <measures-table
          :data="imageMeasures"
          mode="image"
    ...
    <template slot="modal-footer">
      <b-button variant="primary" class="mr-1" @click="onModalSave" v-if="tabtype=='DNA'"
        >Save</b-button
      >
  ...
  methods: {
    ...
    updateData(payload) {
      ...
      if (this.form.image_measurements) {
        this.imageMeasures = this.form.image_measurements.filter(
            m => m.image
          );
      }
    },
