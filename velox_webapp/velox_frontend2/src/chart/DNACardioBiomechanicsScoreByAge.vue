<template>
  <div class="row">
    ...
    <div class="col-lg-10">
      <h5 class="text-center">DNA + Cardio + Biomechanics Score by Age <span><b>{{sum ? sum+' samples' : ''}}</b></span></h5>
      <canvas id="dna_cardio_biomechanics_score_by_age"></canvas>
      <div class="chart1">
      <div class="row text-center">
        <div class="col-lg-12">
          <table class="table table-borderless percantage_all">
              <thead>
                ...
                <tr>
                   ...
                   <th scope="col" v-for="per ,index_4 in  percentageArrEliteofElite" :key="index_4">{{per.perc+'%'}}</th>
                </tr>
                <tr>
          ...
          <label>Measure Type:- </label><input type="radio" v-model="type" value="all" @change="chart_type($event)" checked="type=='all'"/><label for="All">All</label>
          <input type="radio" v-model="type" value="Male Yearling" @change="chart_type($event)"/><label for="Male Yearling">Male Yearling</label>
          <input type="radio" v-model="type" value="Female Yearling" @change="chart_type($event)"/><label for="Female Yearling">Female Yearling</label>
          <input type="radio" v-model="type" value="Female Older" @change="chart_type($event)"/><label for="Female Older">Female Older</label>
          <input type="radio" v-model="type" value="Male 2YO" @change="chart_type($event)"/><label for="Male 2YO">Male 2YO</label>
          <input type="radio" v-model="type" value="Female 2YO" @change="chart_type($event)"/><label for="Female 2YO">Female 2YO</label>
          <input type="radio" v-model="type" value="Male Older" @change="chart_type($event)"/><label for="Male Older">Male Older</label>
          <input type="radio" v-model="type" value="Male Weanling" @change="chart_type($event)"/><label for="Male Weanling">Male Weanling</label>
          <input type="radio" v-model="type" value="Female Weanling" @change="chart_type($event)"/><label for="Female Weanling">Female Weanling</label>
        </div>
      </div>
      </div>
    </div>
...
export default {
  name: 'DNACardioBiomechanicsScoreByAge',
  props: {
		result1:[],
	},
   data() {
  ...
  async mounted() {
    ...
    const ctx = document.getElementById('dna_cardio_biomechanics_score_by_age');
    if (window.bar13 != undefined) {
      window.bar13.destroy();
    }
    const chartData = this.DNACardioBiomechanicsScoreByAge(this.result?.data?.dna_cardio_biomechanics?.age)
    window.bar13 = new Chart(ctx, chartData);
    this.type = "all";
    this.chart_type()
    this.temp1 = this.result?.data?.dna_cardio_biomechanics?.age.map(item => item.counts);
  ...
  methods: {
    async chart_type(event) {
      ...
      let temp,data = [];
      temp = this.result?.data?.dna_cardio_biomechanics?.age;
      ...
      window.bar13 = new Chart(ctx, chart_type_data);
    },
