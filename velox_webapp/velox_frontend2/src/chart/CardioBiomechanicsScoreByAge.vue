<template>
  <div class="row">
    ...
    <div class="col-lg-10">
      <h5 class="text-center">Cardio + Biomechanics Score by Age <span><b>{{sum ? sum+' samples' : ''}}</b></span></h5>
...
export default {
  ...
  async mounted() {
    ...
    const chartData = this.CardioBiomechanicsScoreByAge(this.result?.data?.cardio_biomechanics?.age)
    window.bar11 = new Chart(ctx, chartData);
    this.type = "all";
    this.chart_type()
    this.temp1 = this.result?.data?.cardio_biomechanics?.age.map(item => item.counts);
