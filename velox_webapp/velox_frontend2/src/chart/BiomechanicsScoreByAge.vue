<template>
 <div class="row">
  ...
  <div class="col-lg-10">
    <h5 class="text-center">Biomechanics Score by Age <span><b>{{sum ? sum+' samples' : ''}}</b></span></h5>
    <canvas id="biomechanics_score_by_age"></canvas>
    <div class="chart1">
...
export default {
  ...
  async mounted() {
    ...
    const ctx = document.getElementById('biomechanics_score_by_age');
    if (window.bar6 != undefined) {
      window.bar6.destroy();
    }
    const chartData = this.BiomechanicsScoreByAge(this.result?.data?.biomechanics?.age)
    window.bar6= new Chart(ctx, chartData);
    this.type = "all";
    this.chart_type()
    this.temp1 = this.result?.data?.biomechanics?.age.map(item => item.counts);
  ...
  methods: {
    async chart_type(event) {
      ...
      let temp,data = [];
      temp = this.result?.data?.biomechanics?.age;
      ...
      const chart_type_data =this.BiomechanicsScoreByAge(data)
      this.getPercentage(chart_type_data)
      window.bar6 = new Chart(ctx, chart_type_data);
    },
