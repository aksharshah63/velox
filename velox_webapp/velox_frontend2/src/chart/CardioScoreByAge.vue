<template>
  <div class="row">
    ...
    <div class="col-lg-10">
      <h5 class="text-center">Cardio Score by Age <span><b>{{sum ? sum+' samples' : ''}}</b></span></h5>
      <canvas id="cardio_score_by_age"></canvas>
      <div class="chart1">
...
export default {
  ...
  async mounted() {
    ...
    const ctx = document.getElementById("cardio_score_by_age");
    if (window.bar4 != undefined) {
      window.bar4.destroy();
    }
    const chartData = this.CardioScoreByAge(this.result?.data?.cardio?.age)
    window.bar4 = new Chart(ctx, chartData);
    this.type = "all";
    this.chart_type()
    this.temp1 = this.result?.data?.cardio?.age.map(item => item.counts);
  ...
  methods: {
    async chart_type(event) {
      ...
      let temp,data = [];
      temp = this.result?.data?.cardio?.age;
      ...
      const chart_type_data =this.CardioScoreByAge(data)
      this.getPercentage(chart_type_data)
      window.bar4 = new Chart(ctx, chart_type_data);
    },
